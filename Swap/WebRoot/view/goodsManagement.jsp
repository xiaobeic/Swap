        ...
        <div align="center">
            ...
            <div id="content"  align="left">
                <div id="content_head">
                    ...
                    <a href="${pageContext.request.contextPath}/goods/adminQueryGoods.action"><div class="content_title  content_title_active">物品管理</div></a>
                </div>
                ...
                <div class="content_body" align="center">
                    <div id="content_body_left">
                        <a href="${pageContext.request.contextPath}/goods/adminQueryGoods.action"><div class="content_opertion content_opertion_active">物品列表</div></a>
                        <a href=""><div class="content_opertion">物品创建</div></a>
                    </div>

                    <!-- Recording information start -->
                    <div id="content_body_right">
                        <form id="exam_form" action="${pageContext.request.contextPath}/goods/adminSearchGoods.action" method="post">
                            ...
                            <div id="content_list">
                                <div id="content_list_head">
                                    ...
                                    <span class="exam_title_id" style="width: 7%;">物品ID</span>
                                    <span class="exam_title_id" style="width: 20%;">物品名称</span>
                                    <span class="exam_title_id" style="width: 15%;">上传时间</span>
                                    <span class="exam_title_id" style="width: 10%;">小分类</span>
                                    <span class="exam_title_id" style="width: 15%;">大分类</span>
                                    <span class="exam_title_id" style="width: 15%;">店铺ID</span>
                                    <span class="exam_title_edit" style="width: 7%;">编辑</span>
                                </div>
                                
                                <div style="margin-top: 20px;min-height: 590px;">
                                    <c:if test="${goods.size()==0 }">
                                        <div class="rest"><span>暂无物品信息..</span></div>
                                    </c:if>
                                    <c:if test="${goods.size()!=0 }">
                                        <c:forEach items="${goods }" var="good" varStatus="stat">
                                            <span class="exam_title_id" style="width: 7%;display: inline-block;">${stat.count }</span>
                                            <span class="exam_title_id" style="width: 7%;display: inline-block;">${good.id }</span>
                                            <span class="exam_title_id" style="width: 20%;display: inline-block;">${good.goodsname }</span>
                                            <span class="exam_title_id" style="width: 15%;display: inline-block;">
                                                <fmt:formatDate value="${good.uploadtime }" pattern="yyyy-MM-dd"/>
                                            </span>
                                            <span class="exam_title_id" style="width: 15%;display: inline-block;">${good.smallclassify }</span>
                                            <span class="exam_title_id" style="width: 10%;display: inline-block;">${good.bigclassify }</span>
                                            <span class="exam_title_id" style="width: 15%;display: inline-block;">${good.shopId }</span>
                                            <span class="exam_title_edit" style="width: 7%;display: inline-block;">
                                                <a>更新</a>
                                                <a href="${pageContext.request.contextPath}/goods/adminDeleteGood.action?goodId=${good.id }" class="admin_delete">删除</a>
                                            </span>
                                ...
                                <div id="content_list_footer">
                                    <c:if test="${goods.size()==0 || not empty keyword}">
                                    ...
                                    <a id="homePage" href="${pageContext.request.contextPath}/goods/adminQueryGoods.action?pageNow=1" 
                                        class="button button-border button-rounded  button-primary page_query_a" style="width: 60px;padding: 0px;">首页</a>
                                    <c:choose>
                                        <c:when test="${page.pageNow - 1 > 0}">
                                            <a id="prvePage" href="${pageContext.request.contextPath}/goods/adminQueryGoods.action?pageNow=${page.pageNow - 1}"
                                                class="button   button-border button-rounded button-primary page_query_a">&lt;上一页</a>
                                        </c:when>
                                        <c:when test="${page.pageNow - 1 <= 0}">
                                            <a id="prvePage" href="${pageContext.request.contextPath}/goods/adminQueryGoods.action?pageNow=1"
                                                class="button   button-border button-rounded button-primary page_query_a">&lt;上一页</a>
                                        </c:when>
                                    </c:choose>
                                    <c:choose>
                                        <c:when test="${page.totalPageCount==0}">
                                            <a id="nextPage" href="${pageContext.request.contextPath}/goods/adminQueryGoods.action?pageNow=${page.pageNow}"
                                             class="button   button-border button-rounded button-primary page_query_a">下一页&gt;</a>
                                        </c:when>
                                        <c:when test="${page.pageNow + 1 < page.totalPageCount}">
                                            <a id="nextPage" href="${pageContext.request.contextPath}/goods/adminQueryGoods.action?pageNow=${page.pageNow + 1}"
                                             class="button   button-border button-rounded button-primary page_query_a">下一页&gt;</a>
                                        </c:when>
                                        <c:when test="${page.pageNow + 1 >= page.totalPageCount}">
                                            <a id="nextPage" href="${pageContext.request.contextPath}/goods/adminQueryGoods.action?pageNow=${page.totalPageCount}" 
                                             class="button   button-border button-rounded button-primary page_query_a">下一页&gt;</a>
                                        </c:when>
                                    </c:choose>
                                    <c:choose>
                                        <c:when test="${page.totalPageCount==0}">
                                            <a id="lastPage" href="${pageContext.request.contextPath}/goods/adminQueryGoods.action?pageNow=${page.pageNow}"
                                             class="button   button-border button-rounded button-primary page_query_a" style="width: 60px;padding: 0px;">尾页</a>
                                        </c:when>
                                        <c:otherwise>
                                            <a id="lastPage" href="${pageContext.request.contextPath}/goods/adminQueryGoods.action?pageNow=${page.totalPageCount}"
                                             class="button   button-border button-rounded button-primary page_query_a" style="width: 60px;padding: 0px;">尾页</a>
                                        </c:otherwise>
                                    </c:choose>
                                    <span style="color: #1B9AF7">
                                        ...
                                        <span size="3">第 <input value="${page.pageNow}" id="pageNumTo" size="1">页
                                            <a href="${pageContext.request.contextPath}/goods/adminQueryGoods.action?pageNow=" id="paging_go" class="button  button-border button-rounded button-primary">确定</a>
                                        </span>
